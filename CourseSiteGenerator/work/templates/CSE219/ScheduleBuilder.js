...
function initSchedule() {
    initDateData();
    var dataFile = "./schedule_files/ScheduleData.json";
    loadData(dataFile);
}
...
function buildScheduleTable() {
    ...
    //console.log(countDate.day.toString().length);
    ...
    while (firstDateIsBeforeSecond(countDate, endingFridayDate)) {
        ...
        table.append("<tr>");
        for (var i = 0; i < 5; i++) {
            var displayMonth = (countDate.month.toString().length!=2)? ("0"+countDate.month):(countDate.month);
            var displayDay = (countDate.day.toString().length != 2)? ("0"+countDate.day):(countDate.day);
            table.append(
                    "<td class=\"sch\" id=\"" + displayMonth + "_" + displayDay + "\"><strong>" + countDate.month + "/" + countDate.day + "</strong><br /></td>");
            incDate(countDate);
        }
        table.append("</tr>");
        incDate(countDate);
